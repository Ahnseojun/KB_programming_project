...
namespace MyPCRoom
{
    public partial class Main : Form
    {
        ...
        private void button22_Click_1(object sender, EventArgs e)
        {
            ...
            try
            {
                seat_num = pcNumberLabel.Text.Split('번')[0];
                
                late_time = remainingTimeLabel.Text; // 0:0:5 > 55
                ...
                DataManager.save_and_quite(late_time, seat_num);
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message + " form1 button22 사용종료 error");
            }
            finally
            {
                MessageBox.Show((pcNumberLabel.Text.Split('번')[0]));
                MessageBox.Show(userIdLabel.Text);
                MessageBox.Show(remainingTimeLabel.Text);
                //Timers[int.Parse(pcNumberLabel.Text.Split('번')[0])].Enabled = false;
            }
        }
