...
namespace MyPCRoom
{
    public class DBHelper
    {
        ...
        public static void db_save_quite(string late_time, string seat_num)
        {
            try
            {
                // 사용종료 누르면 db에 저장
                ...
                SqlCommand cmd = new SqlCommand();
                cmd.Connection = conn;  // 어디에 커맨드 보낼지 지정
                cmd.CommandText = "update PCRoom set Late_time = @p2, using_status = 'OFF' where Seat_num='@p1'";

                cmd.Parameters.AddWithValue("@p2", late_time);
